...
ggplot2::ggplot(data = to_plot) +
  ...
  geom_timeline(ggplot2::aes(x = date, y = COUNTRY, i = INTENSITY, d = DEATHS)) +
  ggplot2::geom_segment(data = to_plot2, ggplot2::aes(x = date, xend = date, y = COUNTRY, yend = as.numeric(COUNTRY) + 0.25),
               alpha = 0.5, color = "gray") +
  geom_timeline_label(data = to_plot2, ggplot2::aes(x = date, y = as.numeric(COUNTRY) + 0.4, label = LOCATION_NAME)) +
...
out <- eq_clean_data(out)
out <- dplyr::filter(out, COUNTRY == "MEXICO" & lubridate::year(date) >= 2000)
out <- dplyr::mutate(out, popup_text = eq_create_label(out))
out <- eq_map(out, annot_col = "popup_text")
out
```
